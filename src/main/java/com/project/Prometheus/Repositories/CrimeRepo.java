...
public class CrimeRepo{
    ...
    public List<CrimeEntity> getGroupedCrimes(Filter filter) {
        StringBuilder jpql = new StringBuilder("SELECT c FROM CrimeEntity c WHERE ");
        Map<String, Object> params = new HashMap<>();
        ...
        if (filter.getGardaDivision() != null && filter.getGardaDivision().length > 0) {
            jpql.append("AND c.garda_division IN :gardaDivision ");
            params.put("gardaDivision", Arrays.asList(filter.getGardaDivision()));
        }           

        if (filter.getOffence() != null && filter.getOffence().length > 0) {
            jpql.append("AND c.offence IN :offence ");
            params.put("offence", Arrays.asList(filter.getOffence())); 
       }
